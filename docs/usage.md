# nf-core/pacsomatic: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/pacsomatic/usage](https://nf-co.re/pacsomatic/usage)
...
```bash
nextflow run nf-core/pacsomatic --input ./samplesheet.csv --outdir ./results --fasta ./chr20.fa -profile docker,test
```
...
```bash
nextflow run nf-core/pacsomatic -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nf-core/pacsomatic
```
...
First, go to the [nf-core/pacsomatic releases page](https://github.com/nf-core/pacsomatic/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.

...
workflow METHYLATION_ANALYSIS {
    ...
    // Check if we have phased BAMs
    if (!ch_phased_normal_bam_bai || !ch_phased_tumor_bam_bai) {
        log.warn "Methylation analysis requires phased BAMs from germline and somatic phasing. Skipping."
        ch_versions = Channel.empty()
    } else {
        //
    ...
    emit:
    normal_cpg      = PBCPGTOOLS_NORMAL.out.combined_bed ?: Channel.empty()  // channel: [ meta, bed ]
    tumor_cpg       = PBCPGTOOLS_TUMOR.out.combined_bed ?: Channel.empty()   // channel: [ meta, bed ]
